...
const searchChapter = () =>{//查找学生访问章节记录
  ...
  axios.post("/course/getLogStatistics", formData)
      .then(response => {
        if (response.data.code === 20000) {
          ...
          data.forEach(item => {
            ...
            yAxisData.push(item.countCharpter);
          });
          ...
          // 处理上传失败的情况
          console.error('学生访问章节记录查询正确');
        }
      })
      .catch(error => {
        console.error('学生访问章节记录查询错误', error);
      });
}
const handleBank = () =>{
